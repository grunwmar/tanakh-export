...
"""
    ...
    for i in range(len(books[0])):
        ...
        for j in range(len(books[0][i])):
            ...
            html_verse= html_soup.new_tag('table')
            ...
            for n, b in enumerate(books):
                ...
                html_verse_table_row = html_soup.new_tag('tr')

                html_verse_table_row_lindex = html_soup.new_tag('td')
                html_verse_table_row_lindex['class'] = 'l-index'

                html_verse_table_row_rindex = html_soup.new_tag('td')
                html_verse_table_row_rindex['class'] = 'r-index'

                html_verse_lang = html_soup.new_tag('td')
                ...
                if n == 0:
                    html_verse_table_row_rindex.string = convert_index(lang, verse)
                    html_verse_table_row_lindex.string = str(verse)


                html_verse_table_row.append(html_verse_table_row_lindex)
                html_verse_table_row.append(html_verse_lang)
                html_verse_table_row.append(html_verse_table_row_rindex)

                html_verse.append(html_verse_table_row)
